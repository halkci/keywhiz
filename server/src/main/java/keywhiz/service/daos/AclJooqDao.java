...
class AclJooqDao {
  ...
  @Inject
  public AclJooqDao(DSLContext dslContext, ClientJooqDao clientJooqDao,
  ...
  public Set<Group> getGroupsFor(Secret secret) {
    List<Group> r = dslContext
        .select()
        .from(GROUPS.join(ACCESSGRANTS).on(GROUPS.ID.eq(ACCESSGRANTS.GROUPID))
  ...
  protected ImmutableSet<SecretSeries> getSecretSeriesFor(Group group) {
    List<SecretSeries> r = dslContext
        .select()
        .from(SECRETS.join(ACCESSGRANTS).on(SECRETS.ID.eq(ACCESSGRANTS.SECRETID))
  ...
  protected ImmutableSet<SecretSeries> getSecretSeriesFor(Client client) {
    List<SecretSeries> r = dslContext
        ...
        .from(SECRETS.join(ACCESSGRANTS)
            .on(SECRETS.ID.eq(ACCESSGRANTS.SECRETID))
            .join(MEMBERSHIPS)
            .on(ACCESSGRANTS.GROUPID.eq(MEMBERSHIPS.GROUPID))
            .join(CLIENTS)
  ...
  protected Optional<SecretSeries> getSecretSeriesFor(Client client, String name) {
    // TODO: why does jooq fail to properly type this?
    Record r = dslContext.select()
        .from(SECRETS.join(SECRETS_CONTENT)
            .on(SECRETS.ID.eq(SECRETS_CONTENT.SECRETID))
            .join(ACCESSGRANTS)
            .on(SECRETS.ID.eq(ACCESSGRANTS.SECRETID))
            .join(MEMBERSHIPS)
            .on(ACCESSGRANTS.GROUPID.eq(MEMBERSHIPS.GROUPID))
