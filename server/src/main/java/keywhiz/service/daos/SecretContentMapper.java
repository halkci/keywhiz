...
class SecretContentMapper implements RecordMapper<Record, SecretContent> {
  public SecretContent map(Record record) {
    ...
    Map<String, String> t = gson.fromJson(r.getMetadata(), metadataType);
    ImmutableMap<String, String> metadata;
    if (t != null) {
      metadata = ImmutableMap.copyOf(t);
    } else {
      metadata = ImmutableMap.of();
    }

    return SecretContent.of(r.getId(), r.getSecretid(), r.getEncryptedContent(), r.getVersion(),
        r.getCreatedat(), r.getCreatedby(), r.getUpdatedat(), r.getUpdatedby(), metadata);
  }
}
